 ...
 *
 * OPT-IN, SANDBOX ONLY, NEVER DDL ON PRODUCTION. It runs only with STOCK_CROSSCHECK_DB=1 and
 * TEST_SANDBOX_ORG (an org with an active tech and active office staff; the live companies are
 * refused by id) as well as the TEST_DB_* creds. When 0343/0344 are not on the database it applies
 * them inside the transaction ONLY with STOCK_CROSSCHECK_APPLY=1, which is refused outright for the
 * production project: 0343's trigger DDL locks every company's invoices until the rollback.
 *   TEST_DB_HOST=… TEST_DB_USER=… TEST_DBPW=… STOCK_CROSSCHECK_DB=1 TEST_SANDBOX_ORG=<uuid> \
 *     npx vitest run src/lib/stock-phase3-crosscheck.integration.test.ts --testTimeout=30000
 * After a run, scan for leaked rows (jobs/customers/invoices/stock_* named TEST, created in the last
 * hour); there should be none.
 */

const { TEST_DBPW, TEST_DB_HOST, TEST_DB_USER, STOCK_CROSSCHECK_DB, TEST_SANDBOX_ORG, STOCK_CROSSCHECK_APPLY } = process.env;
const d = TEST_DBPW && TEST_DB_HOST && TEST_DB_USER && STOCK_CROSSCHECK_DB === "1" && TEST_SANDBOX_ORG ? describe : describe.skip;
...
d("Shop Stock Phase 3 cross-check: a crew take, its bell, its bill, its Undo, its void (0343 + 0344)", () => {
  ...
  beforeAll(async () => {
    const sandbox = String(TEST_SANDBOX_ORG ?? "").trim().toLowerCase();
    if (!/^[0-9a-f-]{36}$/.test(sandbox)) throw new Error("stock cross-check: name the sandbox org (TEST_SANDBOX_ORG). It never picks a company by query.");
    if (LIVE_ORGS.has(sandbox)) throw new Error("stock cross-check: that is a live company's org. It writes only in a sandbox org.");
    ...
    if (!fx?.tech_id || !fx?.staff_id) throw new Error("stock cross-check: the sandbox org needs an active tech and active office staff.");
    orgId = sandbox;
    techId = fx.tech_id;
    techName = String(fx.tech_name ?? "").trim() || "A crew member";
    staffId = fx.staff_id;
