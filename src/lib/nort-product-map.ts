...
- Jobs → Panel tab: the job's own circuit list and panel door (spaces, Planned/Roughed/Done, Verified On Site), the Breakers card (what the new circuits need against what came on the tickets), Read The Panel Photo (crew and office, 3 reads a job a day) and Read Circuits From The Plans (office). Everything a reader — or you, with panel.suggest — puts there is a SUGGESTION until a person taps Keep; read it with get_job_panel.
...
- Design studio (/site-studio): redesign the public site by DESCRIBING the change — typed OR spoken (mic button). "Show me options" makes FOUR arrangements in one tap to flip through. The hero's text can be arranged ON the photo (left/center/right; open, translucent panel, or a bottom band) without losing the full-bleed image. "Arrange this version by hand" opens the block editor on a DRAFT (move sections up/down, edit copy), and "Edit on the page" turns the preview itself into an editor: click any text to highlight it (control bar shows fonts/sizes/position), double-click to rewrite it in place, arrow keys micro-adjust the hero text position and headline size — Save lands on the draft, never the live site — each pass makes a numbered VERSION previewed on the real site (draft banner, not public); nothing goes live until Publish, any older version can be re-published (that IS rollback). Designs can only rearrange the org's own photos, never invent images, links off-site, or reviews; addresses/phone/lead capture are untouchable by design.
