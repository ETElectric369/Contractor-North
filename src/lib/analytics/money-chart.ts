 ...
 *
 * Crew Mileage is its own series (the two-bucket law: never folded into crew pay), offered only when
 * the months hold some, the way the card only prints its line when there is one.
 *
...
export function buildMoneyChartData(money: OwnerMoney, opts: { ownerFigures: boolean; leftLabel: string }): MoneyChartData {
  const hasMileage = money.months.some((m) => !tiny(m.crewMileagePaid));
  const keys: MoneySeriesKey[] = opts.ownerFigures
    ? MONEY_SERIES_ORDER.filter((k) => k !== "mileage" || hasMileage)
    : ["collected"];
  ...
  const first = months.findIndex((m) => keys.some((k) => !tiny(m.values[k])));
  return { series, months: first < 0 ? [] : months.slice(first) };
}
...
/** A bar's figure: "$780" under $1,000, "$4.2k", "$21k", "$1.2M". Negative reads "−$47". */
...
export function niceScale(min: number, max: number): { ticks: number[]; lo: number; hi: number } {
  ...
  const span = hi0 - lo0;
  let mag = Math.pow(10, Math.floor(Math.log10(span / 4)));
  for (let guard = 0; guard < 12; guard++, mag *= 10) {
    for (const m of [1, 2, 2.5, 5]) {
      const step = m * mag;
      const nPos = Math.ceil(hi0 / step - 1e-9);
...
export function layoutMoneyChart(months: MoneyChartMonth[], on: MoneySeriesKey[]): ChartLayout {
  ...
  for (let pass = 0; pass < 4; pass++) {
    const k = scale.hi - scale.lo > 0 ? PLOT_H / (scale.hi - scale.lo) : 0;
    const zeroY = top + scale.hi * k;
    const plotBottom = top + (scale.hi - scale.lo) * k;
    ...
    out = {
      ...
      barWidth,
      minGroupWidth,
      minPlotWidth: Math.max(1, months.length) * minGroupWidth,
      axisWidth,
      monthY,
      yearY,
    };
