 ...
 *  failure is not logged to reminder_log (so it retries next run) but IS reported to
 *  error_events and counted (audit v921 — a rotated Resend key used to fail every reminder
 *  for weeks in total silence). Nothing sends for an org whose toggles are off. */

type Counts = { invoice_due: number; quote_followup: number; appointment: number; skipped_no_email: number; failed: number };
const DAY = 86_400_000;

export async function sendDueReminders(supabase: any): Promise<Counts> {
  const now = Date.now();
  const counts: Counts = { invoice_due: 0, quote_followup: 0, appointment: 0, skipped_no_email: 0, failed: 0 };
  ...
  for (const org of orgs ?? []) {
    try {
      ...
      // A REJECTED SEND IS NOT A NON-EVENT (audit v921). Resend answering 401 (rotated key),
      // 422 (unverified branded sender) or 429 just left the loop moving on: nothing in
      // error_events, nothing in the counts, cron still {ok:true} — every reminder for every org
      // could fail for weeks and the daily ops query would show a clean board.
      function sendFailed(kind: string, entityId: string, error?: string): void {
        counts.failed++;
        reportError("reminder-send", new Error(error || "send failed"), { orgId: org.id, kind, entityId });
      }
      ...
      if (s.remind_invoice_due) {
        ...
        for (const inv of invs ?? []) {
          ...
          if (!r.ok) sendFailed("invoice_due", inv.id, r.error);
          else if (await logSent("invoice_due", inv.id)) counts.invoice_due++;
        }
      }
      ...
      if (s.remind_quote_followup) {
        ...
        for (const q of qs ?? []) {
          ...
          if (!r.ok) sendFailed("quote_followup", q.id, r.error);
          else if (await logSent("quote_followup", q.id)) counts.quote_followup++;
        }
      }
      ...
      if (s.remind_appointments) {
        ...
        for (const a of appts ?? []) {
          ...
          if (!r.ok) sendFailed("appointment", a.id, r.error);
          else if (await logSent("appointment", a.id)) counts.appointment++;
        }
      }
    } catch (e) {
      // One org's thrown send (DNS/network) used to abort the whole loop, so every org after it
      // was skipped for that run (audit v921). Log it and carry on to the next tenant.
      reportError("reminders-org", e, { orgId: org.id });
    }
  }
  return counts;
}
