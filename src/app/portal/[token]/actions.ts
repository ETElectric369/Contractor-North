...
export async function sendPortalCode(token: string): Promise<SendCodeResult> {
  ...
  if (!r?.ok) {
    const reason = (["no_link", "no_email", "too_many", "channel_unavailable"].includes(String(r?.reason))
      ? r!.reason
      : "send_failed") as SendRefusal;
    return reason === "too_many" ? { ok: false, reason, retryMinutes: minutesUntil(r?.retry_at) } : { ok: false, reason };
  }
  if (!r.to) return { ok: false, reason: "send_failed" };

  const sent = await deliverPortalCode({
  ...
  if (!sent.ok) {
    // Nothing silent: the office hears it (error_events), the customer reads "couldn't send".
