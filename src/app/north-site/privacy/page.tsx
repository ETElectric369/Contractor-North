 ...
 *                  (org setting geofence_logout), app/api/weather/route.ts (rounded), Info.plist has
 *                  WhenInUse only and no UIBackgroundModes location
...
const PERMISSIONS: { name: string; why: string; when: string }[] = [
  { name: "Location (while using)", why: "Saves where you clocked in and out, shows local weather, and, if your company turns it on, checks you are still at the job site.", when: "the first time you clock in or tap Use My Location." },
...
const PROCESSORS: { who: string; what: string }[] = [
  ...
  { who: "Anthropic (Claude)", what: "Runs Nort. Gets what you ask Nort and the records it looks up to answer, which can include customer names, addresses, hours and pay. Also reads receipts, supplier invoices, plans and documents you ask North to read, and chat from a contractor's website assistant." },
  ...
  { who: "Stripe", what: "Payments. Card numbers are typed on Stripe's own page or read by Stripe's card reader, never stored by North. Gets amounts, invoice details, the payer's email, and the business details a contractor gives Stripe to get paid." },
    ...
    <>
      ...
      <Section title="What North collects">
        ...
        <ul>
          ...
          <li>Leads from a contractor&apos;s website: the form message, photos and plans people upload, and chat with the website&apos;s assistant. Photos uploaded through a website lead form are stored at an unguessable web address, so anyone who has that exact address can open them.</li>
