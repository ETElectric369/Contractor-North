...
/**
 * Public marketing homepage for an org's Contractor-North-hosted site at /site/<handle>.
 * 100% data-driven from the org record + settings (name, logo, phone, license, glass_tint,
 * splash_*, portfolio) — the SAME template every org reuses; Tahoe Deck and ET Electric just
 * carry different data. The estimate configurator is the primary CTA. Technical SEO (title,
 * meta, OpenGraph, LocalBusiness JSON-LD) is baked in so every hosted site is indexable.
 */
export async function generateMetadata({ params }: { params: Promise<{ handle: string }> }): Promise<Metadata> {
  ...
  const title = `${org.name} — ${s.splash_headline || "Custom deck builder"}`;
  const description =
    s.splash_tagline || `${org.name} — quality craftsmanship. Get a free instant estimate.`;
        ...
        <div className="mx-auto max-w-6xl px-4 py-24 sm:py-32">
          <div className="max-w-2xl">
            ...
            <h1 className="text-4xl font-extrabold leading-tight tracking-tight text-white drop-shadow sm:text-5xl">
              {s.splash_headline || `${org.name} — custom decks built right`}
            </h1>
