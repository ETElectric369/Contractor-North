...
export default async function RecurringPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader title="Recurring" description="Jobs and expenses that repeat — generate them with one tap.">
            ...
            {(templates ?? []).map((t: any) => {
              ...
              return (
                <li key={t.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
                  <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${t.kind === "job" ? "bg-brand/10 text-brand" : "bg-amber-50 text-amber-600"}`}>
                    {t.kind === "job" ? <Briefcase className="h-4 w-4" /> : <Wallet className="h-4 w-4" />}
                  </span>
