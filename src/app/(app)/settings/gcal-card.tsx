...
export function GcalCard({
  ...
  /** Google rejected the refresh token (revoked/expired grant) — sync is
   *  paused until the user reconnects. */
  needsReauth?: boolean;
}) {
  ...
  return (
    <div className="space-y-3">
      ...
      {connected ? (
        <>
          {/* Dead grant (Google revoked/expired the refresh token): sync is
              paused — say so and lead with the reconnect, not a green badge. */}
          {needsReauth && (
            <div className="flex flex-wrap items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
              <span className="text-sm text-amber-800">
                Google signed this connection out, so syncing is paused. Reconnect to resume — your
                calendar picks are kept.
              </span>
              <a
              ...
              >
                <CalendarCheck className="h-4 w-4 shrink-0" /> Reconnect
              </a>
            </div>
          )}
          <div className="flex flex-wrap items-center gap-3">
            {needsReauth ? <Badge tone="amber">Reconnect needed</Badge> : <Badge tone="green">Connected</Badge>}
            {lastSyncedAt && (
