-- The public /i/<token> invoice page classifies line items into the Labor/Materials
-- Cost Breakdown by import_source, falling back to the importer's "Labor — "/"Materials — "
-- description prefix. The itemized costs import (cn-v545) writes raw product descriptions
-- with no prefix, so on the public page — whose RPC didn't expose import_source — every
-- material line fell into "Other". Expose the bucket tag (it's just "labor"/"costs"/
-- "draw_credit"/"milestone"; no tenant data) so the public page classifies exactly like
-- the app and print surfaces.
--
-- Rewritten from the LIVE definition (pg_get_functiondef, 2026-07-24) — never from an
-- older migration copy.
